...
?>
<?php
$sql = mysqli_query($conn,"select * from vehiclestore where vehicleid='$_GET[vahicleid]'");
...
?>
		...
		<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1" onlclick="return validate()">		
			<table width="325", border="0">
			  ...
			  <tr>
			    <th scope="row">&nbsp;</th>
			    <td><?php if(isset($_GET['vahicleid'])) { ?> <input type="submit" name="update" id="submit" value="Update Vehicle" /> <?php } else { ?> <input type="submit" name="submit" id="submit" value="Register" /><?php } ?></td>
		      </tr>
          </table>
          <?php
			}
